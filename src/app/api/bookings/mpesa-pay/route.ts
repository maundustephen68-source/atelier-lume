...
export async function POST(req: Request) {
  ...
  try {
    const stk = await initiateStkPush({
      ...
      amount: Math.round(service.price * (Number(process.env.DEPOSIT_PERCENT || 100) / 100)),
      accountReference: booking.id,
      description: service.name,
      callbackUrl: `${site}/api/webhooks/mpesa`,
    });
    ...
    return NextResponse.json({ bookingId: booking.id, checkoutRequestId: stk.checkoutRequestId });
  } catch (err) {
    await prisma.booking.update({ where: { id: booking.id }, data: { status: "cancelled" } });
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Could not start M-Pesa payment. Please try again." },
      { status: 502 }
    );
  }
}